...
public class CatacombLayerGenerator extends LayerGenerator {
    ...
    public static final CatacombLayerGenerator INSTANCE = new CatacombLayerGenerator();

    private int nodesLeft;
//    private int roomsLeft;

    public CatacombLayerGenerator() {
        super();
    }
...
//        this.roomsLeft = settings.rooms.generateInt(rand);
    ...
    public void generateLayer(DungeonBuilder dungeonBuilder, DungeonLayer dungeonLayer, int layer, Random rand, Position2D start) {
        ...
        dungeonLayer.grid[bottomStairs.gridPosition.x][bottomStairs.gridPosition.z] = new PlaceHolder(bottomStairs).addFlag(PlaceHolder.Flag.FIXED_ROTATION);
        ...
        dungeonLayer.grid[topStairs.gridPosition.x][topStairs.gridPosition.z] = new PlaceHolder(topStairs).addFlag(PlaceHolder.Flag.FIXED_ROTATION);
    ...
    private Position2D createCorridors(DungeonLayer dungeonLayer, Random rand, Position2D origin) {
        ...
        Direction[] directions = Orientation.HORIZONTAL_FACINGS;
    ...
    private Tuple<Position2D, Integer> catacombCorridor(DungeonLayer dungeonLayer, Random rand, Position2D origin, Direction direction, int depth) {
        ...
        while (cursor.isValid(dungeonLayer.width, dungeonLayer.length) && currentLength < length) {
            if (!dungeonLayer.isTileFree(cursor)) {
                ...
                if (depth < settings.maxDepth && currentLength > 3 && rand.nextFloat() < (0.4F - depth * 0.15F)) {
