...
public class DungeonStairs extends DungeonPiece {
	...
	public boolean addComponentParts(IWorld worldIn, Random randomIn, MutableBoundingBox structureBoundingBoxIn,
		...
		switch (stairType) {
		case 0: {
			DungeonSegmentModel model = DungeonSegmentModelRegistry.STAIRS;
			if (model == null)
				return false;
		...
		case 1: {
			DungeonSegmentModel model = DungeonSegmentModelRegistry.STAIRS_BOTTOM;
			if (model == null)
				return false;
		...
		case 2: {
			DungeonSegmentModel model = DungeonSegmentModelRegistry.STAIRS_TOP;
			if (model == null)
				return false;
			build(model, worldIn, new BlockPos(x, y, z), Theme.get(theme), Theme.getSub(subTheme),
					Treasure.Type.DEFAULT, stage, false);
	...
	public void addWalls(DungeonPiece piece, IWorld world, Theme theme) {
		if (!piece.sides[0])
			for (int x = 2; x < 6; x++)
				for (int y = 2; y < 6; y++)
					piece.setBlockState(theme.solid.get(), world, null, piece.x + x, piece.y + y, piece.z, this.theme,
							0, true);
		else
			for (int x = 2; x < 6; x++)
				for (int y = 2; y < 6; y++)
					if (!world.getBlockState(new BlockPos(piece.x + x, piece.y + y, piece.z)).isSolid())
						piece.setBlockState(Blocks.IRON_BARS.getDefaultState(), world, null, piece.x + x, piece.y + y,
								piece.z, this.theme, 0, true);
		if (!piece.sides[1])
			for (int z = 2; z < 6; z++)
				for (int y = 2; y < 6; y++)
			...
			for (int z = 2; z < 6; z++)
				for (int y = 2; y < 6; y++)
					if (!world.getBlockState(new BlockPos(piece.x + 7, piece.y + y, piece.z + z)).isSolid())
						piece.setBlockState(Blocks.IRON_BARS.getDefaultState(), world, null, piece.x + 7, piece.y + y,
			...
			for (int x = 2; x < 6; x++)
				for (int y = 2; y < 6; y++)
					if (!world.getBlockState(new BlockPos(piece.x + x, piece.y + y, piece.z + 7)).isSolid())
						piece.setBlockState(Blocks.IRON_BARS.getDefaultState(), world, null, piece.x + x, piece.y + y,
								piece.z + 7, this.theme, 0, true);
		if (!piece.sides[3])
			for (int z = 2; z < 6; z++)
				for (int y = 2; y < 6; y++)
					piece.setBlockState(theme.solid.get(), world, null, piece.x, piece.y + y, piece.z + z, this.theme,
							0, true);
		else
			for (int z = 2; z < 6; z++)
				for (int y = 2; y < 6; y++)
