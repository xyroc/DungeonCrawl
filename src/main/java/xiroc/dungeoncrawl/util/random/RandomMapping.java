...
public class RandomMapping<V> {
    ...
    public Delegate<V> roll(ResourceLocation key, RandomSource random) {
        if (key == null) {
            ...
            if (object.has(KEY_MAPPING)) {
                ...
                mapping.entrySet().forEach((entry) -> {
                    if (entry.getKey().startsWith(TAG_PREFIX)) {
                        ResourceLocation tagKey = ResourceLocation.parse(entry.getKey().substring(1));
                        builder.tagEntries.put(tagKey, context.deserialize(entry.getValue(), builderType));
                        return;
                    }
                    final var key = ResourceLocation.parse(entry.getKey());
