...
public class DatapackRegistry<T> {
    ...
    public void reload(ResourceManager resourceManager) {
        ...
        resourceManager.listResources(directory.path(), path -> path.getPath().endsWith(FILE_ENDING)).forEach((location, resource) -> {
            try {
                ResourceLocation key = directory.key(location, FILE_ENDING);
                T value = parser.parse(resourceManager, key, new InputStreamReader(resource.open()));
                builder.put(key, value);
            } catch (Exception exception) {
                throw new DatapackLoadException("Failed to load " + location.toString() + ": " + exception.getMessage());
            }
        });
        ...
        return new Codec<>() {
            ...
            public <A> DataResult<Pair<Delegate<T>, A>> decode(DynamicOps<A> ops, A input) {
                return Codec.STRING.decode(ops, input).flatMap( key -> {
                    try {
                        return DataResult.success(Pair.of(delegateOrThrow(ResourceLocation.parse(key.getFirst())), key.getSecond()));
                    } catch (Exception e) {
