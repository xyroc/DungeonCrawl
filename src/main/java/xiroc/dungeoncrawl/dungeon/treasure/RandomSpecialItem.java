...
public class RandomSpecialItem {
	...
	static {
		...
		IRON_SWORD = new TreasureEntry("minecraft:air", 1).withProcessor((world, rand, theme, lootLevel) -> SpecialItemTags
	...
	public static ItemStack generate(ServerWorld world, Random rand, Integer theme, Integer lootLevel) {
		...
		return rand.nextDouble() < 0.8 ? SPECIAL_ITEMS[rand.nextInt(SPECIAL_ITEMS.length)].generate(world, rand, theme, lootLevel)
